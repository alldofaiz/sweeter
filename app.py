...
# kunci rahasia untuk enkripsi dan deskripsi
SECRET_KEY = 'SPARTA'
...
def user(username):
    ...
    # pengecualian ini beralih ke beranda
    except (jwt.ExpiredSignatureError, jwt.exceptions.DecodeError):
        return redirect(url_for('home'))

# post adalah route untuk mengembalikan json
...
def sign_up():
    ...
    # fungsi mengacak password menggunakan hash
    # utf-8 untuk mengurangi error
    # hexdigest adalah string mentahan untuk katasandi
...
# untuk mengetes identitas rangkap
@app.route('/sign_up/check_dup', methods=['POST'])
def check_dup():
    # mengestrak nama pengguna dari permintaan
    username_receive = request.form['username_give']
...
def update_profile():
    # pengguna harus masuk jadi harus menerima token dari peramban
    token_receive = request.cookies.get(TOKEN_KEY)
    try:
        payload = jwt.decode(
            token_receive,
